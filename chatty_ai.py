...
class ChattyAIWebServer:
    ...
    def __init__(self):
        ...
        # Camera - using subprocess approach, no picam2 object needed
    ...
    def setup_camera(self):
        """Initialize camera using subprocess approach that works with your setup"""
        try:
            # Test if cam utility works with basic capture
            test_command = [
            ...
            if result.returncode == 0:
                ...
                self.emit_log("Camera initialized successfully using cam utility", 'success')
                # Clean up test file
                try:
                ...
                self.emit_log(f"Camera test failed: {result.stderr}", 'error')
                self.camera_initialized = False
                return False
    ...
    def capture_camera_frame(self):
        """Capture a single frame using the subprocess approach"""
        try:
            import tempfile
            ...
            # Capture frame using cam utility
            capture_command = [
            ...
            result = subprocess.run(
                ...
                capture_output=True,
                text=True,
                timeout=7  # Slightly longer than the timeout command
            )
            ...
            if result.returncode == 0 and os.path.exists(temp_filename):
                ...
                if frame is not None:
                    ...
                    self.emit_log("Captured image could not be loaded", 'error')
                    return None
            else:
                self.emit_log(f"Camera capture failed: {result.stderr}", 'error')
                # Clean up on failure
                try:
                    os.remove(temp_filename)
                except:
            ...
            self.emit_log(f"Camera capture error: {e}", 'error')
            return None
        ...
        while self.system_running and self.monitor_running:
            try:
                if not self.camera_initialized:
                    ...
                    # Capture frame using subprocess approach
    ...
    def cleanup(self):
        ...
        # Reset camera state
        self.camera_initialized = False
        logger.info("Camera state reset")
